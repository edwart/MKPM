# This Makefile is for the MKPM extension to perl.
#
...
#     CONFIGURE_REQUIRES => {  }
#     DISTNAME => q[MKPM]
#     EXE_FILES => [q[script/mkpm_cgi.pl], q[script/mkpm_create.pl], q[script/mkpm_fastcgi.pl], q[script/mkpm_server.pl], q[script/mkpm_test.pl]]
#     LICENSE => q[perl]
#     NAME => q[MKPM]
#     NO_META => q[1]
#     PREREQ_PM => { Catalyst::View::TT=>q[0], namespace::autoclean=>q[0], Catalyst::Plugin::Static::Simple=>q[0], ExtUtils::MakeMaker=>q[6.36], Catalyst::Plugin::ConfigLoader=>q[0], Catalyst::Action::RenderView=>q[0], Test::More=>q[0.88], Config::General=>q[0], Catalyst::Runtime=>q[5.90011], Moose=>q[0] }
#     VERSION => q[0.01]
#     VERSION_FROM => q[lib/MKPM.pm]
#     dist => {  }
#     realclean => { FILES=>q[MYMETA.yml] }
#     test => { TESTS=>q[t/01app.t t/02pod.t t/03podcoverage.t t/controller_Hello.t t/view_TT.t] }
...
O_FILES  = 
H_FILES  = 
MAN1PODS = script/mkpm_cgi.pl \
	script/mkpm_create.pl \
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=1 --perm_rw=$(PERM_RW) \
	  ...
	  script/mkpm_create.pl $(INST_MAN1DIR)/mkpm_create.pl.$(MAN1EXT) \
	  script/mkpm_cgi.pl $(INST_MAN1DIR)/mkpm_cgi.pl.$(MAN1EXT) \
...
EXE_FILES = script/mkpm_cgi.pl script/mkpm_create.pl script/mkpm_fastcgi.pl script/mkpm_server.pl script/mkpm_test.pl

pure_all :: $(INST_SCRIPT)/mkpm_test.pl $(INST_SCRIPT)/mkpm_create.pl $(INST_SCRIPT)/mkpm_cgi.pl $(INST_SCRIPT)/mkpm_server.pl $(INST_SCRIPT)/mkpm_fastcgi.pl
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/mkpm_test.pl $(INST_SCRIPT)/mkpm_create.pl \
	  $(INST_SCRIPT)/mkpm_cgi.pl $(INST_SCRIPT)/mkpm_server.pl \
	  $(INST_SCRIPT)/mkpm_fastcgi.pl 

$(INST_SCRIPT)/mkpm_test.pl : script/mkpm_test.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/mkpm_test.pl
	$(CP) script/mkpm_test.pl $(INST_SCRIPT)/mkpm_test.pl
	$(FIXIN) $(INST_SCRIPT)/mkpm_test.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/mkpm_test.pl

$(INST_SCRIPT)/mkpm_create.pl : script/mkpm_create.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/mkpm_create.pl
	$(CP) script/mkpm_create.pl $(INST_SCRIPT)/mkpm_create.pl
	$(FIXIN) $(INST_SCRIPT)/mkpm_create.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/mkpm_create.pl

$(INST_SCRIPT)/mkpm_cgi.pl : script/mkpm_cgi.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/mkpm_cgi.pl
	$(CP) script/mkpm_cgi.pl $(INST_SCRIPT)/mkpm_cgi.pl
	$(FIXIN) $(INST_SCRIPT)/mkpm_cgi.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/mkpm_cgi.pl

$(INST_SCRIPT)/mkpm_server.pl : script/mkpm_server.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/mkpm_server.pl
	$(CP) script/mkpm_server.pl $(INST_SCRIPT)/mkpm_server.pl
	$(FIXIN) $(INST_SCRIPT)/mkpm_server.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/mkpm_server.pl

$(INST_SCRIPT)/mkpm_fastcgi.pl : script/mkpm_fastcgi.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/mkpm_fastcgi.pl
	$(CP) script/mkpm_fastcgi.pl $(INST_SCRIPT)/mkpm_fastcgi.pl
	$(FIXIN) $(INST_SCRIPT)/mkpm_fastcgi.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/mkpm_fastcgi.pl
...
TEST_FILE = test.pl
TEST_FILES = t/01app.t t/02pod.t t/03podcoverage.t t/controller_Hello.t t/view_TT.t
TESTDB_SW = -d
...
catalyst_par :: all
	$(NOECHO) $(PERL) -Ilib -Minc::Module::Install -MModule::Install::Catalyst -e"Catalyst::Module::Install::_catalyst_par( '', 'MKPM', { CLASSES => [], PAROPTS =>  {}, ENGINE => 'CGI', SCRIPT => '', USAGE => q## } )"
...
upgradedeps ::
	$(PERL) Makefile.PL --config= --upgradedeps=Test::More,0.88,Catalyst::Runtime,5.90011,Catalyst::Plugin::ConfigLoader,0,Catalyst::Plugin::Static::Simple,0,Catalyst::Action::RenderView,0,Catalyst::View::TT,0,Moose,0,namespace::autoclean,0,Config::General,0

upgradedeps_notest ::
	$(PERL) Makefile.PL --config=notest,1 --upgradedeps=Test::More,0.88,Catalyst::Runtime,5.90011,Catalyst::Plugin::ConfigLoader,0,Catalyst::Plugin::Static::Simple,0,Catalyst::Action::RenderView,0,Catalyst::View::TT,0,Moose,0,namespace::autoclean,0,Config::General,0

listdeps ::
	@$(PERL) -le "print for @ARGV" 

listalldeps ::
	@$(PERL) -le "print for @ARGV" Test::More Catalyst::Runtime Catalyst::Plugin::ConfigLoader Catalyst::Plugin::Static::Simple Catalyst::Action::RenderView Catalyst::View::TT Moose namespace::autoclean Config::General
